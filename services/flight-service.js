...
export async function getNearestAirportIATACode(location) {
  ...
  const nearestIATA = await getNearestAirportIATA(foundLocation.latitude, foundLocation.longitude);
...
export async function getFlightOffers({ origin, destination, departureDate, returnDate, serpApiKey, test }) {
  ...
  const formattedDepartureDate = isValidDateFormat(departureDate) ? departureDate : parseRelativeWeek(departureDate);
  const formattedReturnDate = isValidDateFormat(returnDate) ? returnDate : parseRelativeWeek(returnDate);
