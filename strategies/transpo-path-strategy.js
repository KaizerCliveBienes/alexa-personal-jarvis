...
class TranspoPathStrategy extends Strategy {
  ...
  async execute({ from, to, date, time }) {
    const fromLocationId = await this.fetchNearestLocationId(await this.fetchLocation(from));
    const toLocationId = await this.fetchNearestLocationId(await this.fetchLocation(to));
    const dateTime = this.parseDepartureTime(date, time);


    return this.formatResponse(await this.fetchJourney(fromLocationId, toLocationId, dateTime), date, time);
  }
  ...
  async fetchJourney(fromLocationId, toLocationId, dateTime) {
    ...
    const response = await fetch(`https://v6.bvg.transport.rest/journeys?from=${fromLocationId}&to=${toLocationId}&results=1${dateTime}`, {
      method: 'GET',
    });
  ...
  async fetchNearestLocationId(location) {
    ...
    const response = await fetch(`https://v6.bvg.transport.rest/locations/nearby?latitude=${location.lat}&longitude=${location.lng}&results=1`, {
      method: 'GET',
    });
  ...
  async fetchLocation(location) {
    ...
    const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(location)}+Berlin,+Germany&key=${this.googleMapsApiKey}`, {
      method: 'GET',
    })

    if (!response.ok) {
      throw new Error('Can\'t fetch value in google maps for location: ' + location);
    }

    const responseJson = await response.json();
    if (!('results' in responseJson) || responseJson.results.length === 0) {
      throw new Error('Can\'t find any results in google maps for location: ' + location);
    }
  ...
  formatResponse(content, date, time) {
    ...
    const formatTransportationType = (currentLeg, destination, isShort) => {
      ...
      if (isShort) {
        return currentLeg.line
          ...
          : (currentLeg.walking ? `walk for ${currentLeg.distance ?? ''} meters to ${destination}. ` : '');
      }
    ...
    const legsFormatted = content.legs.reduce((accumulator, currentValue, legIndex) => {
      const origin = modifyLocationName(currentValue.origin.name ?? '');
      const destination = modifyLocationName(currentValue.destination.name ?? '');

      const travelTimeInMinutes = getDifferenceInMinutes(currentValue.plannedDeparture, currentValue.plannedArrival);
    ...
    let departureTimeMessage = date || time ? `If you leave at ${date ?? ''} ${time ?? ''}` : '';
    return {
      version: '1.0',
      response: {
        outputSpeech: {
