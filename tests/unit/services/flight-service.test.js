...
describe("Flight Service", () => {
  ...
  describe("getNearestAirportIATACode", () => {
    it("should return the nearest IATA code for a given location", async () => {
      ...
      const mockResponse = [{ lat: "52.5200", lon: "13.4050" }];
      ...
      expect(fetch).toHaveBeenCalledWith(
        `https://nominatim.openstreetmap.org/search?city=${encodeURIComponent(mockLocation)}&format=json`,
        {
          method: "GET",
        },
      );
    ...
    it("should throw an error if no location is found", async () => {
      ...
      await expect(getNearestAirportIATACode(mockLocation)).rejects.toThrow(
        "No nearest place retrieved: " + mockLocation,
      );
    });
  });
  ...
  describe("getFlightOffers", () => {
    it("should fetch flight offers and return the response", async () => {
      ...
      vi.mock("serpapi", () => ({
        getJson: vi
          .fn()
          .mockResolvedValue({ best_flights: [], other_flights: [] }),
      }));
